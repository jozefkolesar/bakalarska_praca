...
?>
 ...
 <?php
   $db_spojenie = mysqli_connect('127.0.0.1', 'root','','db_kviz','3306');
    ...
    $objekt_vysledkov = mysqli_query($db_spojenie, 'SELECT * FROM uzivatelia_ss ORDER BY Body DESC LIMIT 10');
		if (!$objekt_vysledkov) die ('Chyba v príkaze SQL: ' . mysqli_error($db_spojenie));

    echo('<h2 class = "heading">TOP 10 užívateľov - stredoškolákov/gymnazistov podľa bodov</h2>');
